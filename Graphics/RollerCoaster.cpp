...
RollerCoaster::RollerCoaster(SDL_Surface *r,string s){
	...
	screen=r;
	int nBut=7;
	mainMenu = new Menu(r,20,20,nBut,20);
	mainMenu->toggle(0);
	mainMenu->toggle(1);
	currOffsetX=0;
	currOffsetY=0;
...
void RollerCoaster::drawCurrent(){
	...
	mainMenu->draw(screen,menuBackPNG);
	for(vector<TrackPiece>::iterator it = track.begin();it!=track.end();it++){
...
void RollerCoaster::checkPlace(Cart *myCart){
	...
	double dist2= distance(track[myCart->currTrack].startX,track[myCart->currTrack].startY,x1,y1);
	...
	myCart->angle=track[myCart->currTrack].findAngle();
	myCart->direction=track[myCart->currTrack].findAngle();
	...
	myCart->x=track[myCart->currTrack].startX+(myCart->distTrack)*dist*cos(myCart->angle)+CH*cos(myCart->angle+M_PI/2)/2;
	myCart->y=track[myCart->currTrack].startY-(myCart->distTrack)*dist*sin(myCart->angle)-CH*sin(myCart->angle+M_PI/2)/2;
}
...
void RollerCoaster::play(){
	...
	myCart.draw(currOffsetX,currOffsetY);
	myCart.speed=.15;
	SDL_Flip(screen);
	SDL_Delay(200);
	for(int F=0;F<350;F++){
